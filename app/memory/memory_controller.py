...
class UnifiedMemoryController:
    ...
    def __init__(
        ...
        authority_resolver: Optional[MemoryAuthorityResolver] = None,
    ) -> None:
    ...
    async def get_unified_context(
        ...
        *,
        ...
        async with lock:
            ...
            snapshot.conversation_history = await self._get_conversation(
    ...
    def _extract_all_facts(
        ...
        facts.extend(
            extract_facts_from_conversation(snapshot.conversation_history)
        )
        facts.extend(
            extract_facts_from_vector_context(snapshot.vector_context)
        )
    ...
    def _merge_contexts(self, snapshot: UnifiedMemorySnapshot) -> str:
        ...
        trusted_kg = [
            f for f in snapshot.kg_facts
            if f.get("trust_score", 1.0) >= self._resolver.kg_confidence_threshold
        ]
        if trusted_kg:
            facts_text = "\n".join(
                f"{f['subject']} → {f['relation']} → {f['object']}"
                for f in trusted_kg[:10]
            )
            parts.append(f"--- KNOWLEDGE GRAPH FACTS (confidence ≥ "
                         f"{self._resolver.kg_confidence_threshold}) ---\n{facts_text}")

        # Append authoritative resolved facts section
        if snapshot.resolved_facts:
            resolved_lines = [
                f"{key} = {fact.value}  (source: {fact.source.name}, "
