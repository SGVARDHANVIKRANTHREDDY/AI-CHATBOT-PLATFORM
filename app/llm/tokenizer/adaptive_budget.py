...
class AdaptiveTokenBudgeter:
    ...
    def calculate_budget(
        self, system_prompt: str, user_query: str, memory_content: str, rag_context: str
    ) -> dict[str, int]:
        """
        ...
        if remaining <= 0:
            ...
            return {"system": system_tokens, "user": user_tokens, "memory": 0, "rag": 0}

        # 2. Allocating remaining tokens (60% RAG, 40% Memory)
        rag_budget = int(remaining * 0.6)
        memory_budget = remaining - rag_budget

        return {"system": system_tokens, "user": user_tokens, "memory": memory_budget, "rag": rag_budget}
