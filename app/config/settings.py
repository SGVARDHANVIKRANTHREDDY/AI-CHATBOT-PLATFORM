...
class Settings(BaseSettings):
    ...
    # Storage Paths
    UPLOADED_DOCS_DIR: Path = Field(default_factory=lambda: Path(__file__).resolve().parent.parent.parent / "data" / "raw_docs")
    PROCESSED_DOCS_DIR: Path = Field(default_factory=lambda: Path(__file__).resolve().parent.parent.parent / "data" / "processed_docs")
    VECTOR_INDEX_DIR: Path = Field(default_factory=lambda: Path(__file__).resolve().parent.parent.parent / "data" / "vector_index")
    ...
    HF_API_URL: Optional[str] = Field(default=None, json_schema_extra={"env": "HF_API_URL"})
    ...
    POSTGRES_URL: str = Field(default="postgresql://user:password@localhost:5432/chatbot", json_schema_extra={"env": "POSTGRES_URL"})
    ...
    # Request Protection
    MAX_REQUEST_BODY_BYTES: int = Field(default=1048576, gt=0, json_schema_extra={"env": "MAX_REQUEST_BODY_BYTES"})  # 1 MB
    ...
    CONTENT_SAFETY_INJECTION_THRESHOLD: float = Field(default=0.6, ge=0.0, le=1.0, json_schema_extra={"env": "CONTENT_SAFETY_INJECTION_THRESHOLD"})
    CONTENT_SAFETY_QUARANTINE_THRESHOLD: float = Field(default=0.35, ge=0.0, le=1.0, json_schema_extra={"env": "CONTENT_SAFETY_QUARANTINE_THRESHOLD"})
    CONTENT_SAFETY_QUALITY_FLOOR: float = Field(default=0.3, ge=0.0, le=1.0, json_schema_extra={"env": "CONTENT_SAFETY_QUALITY_FLOOR"})

    # Performance Layer
    SEMANTIC_CACHE_ENABLED: bool = Field(default=True, json_schema_extra={"env": "SEMANTIC_CACHE_ENABLED"})
    SEMANTIC_CACHE_THRESHOLD: float = Field(default=0.92, ge=0.0, le=1.0, json_schema_extra={"env": "SEMANTIC_CACHE_THRESHOLD"})
    ...
    RERANKER_MODEL: str = Field(default="cross-encoder/ms-marco-MiniLM-L-6-v2", json_schema_extra={"env": "RERANKER_MODEL"})
    ...
    MODEL_REASONING: str = Field(default="mistralai/Mistral-7B-Instruct-v0.2", json_schema_extra={"env": "MODEL_REASONING"})
    MODEL_CODING: str = Field(default="codellama/CodeLlama-13b-Instruct-hf", json_schema_extra={"env": "MODEL_CODING"})
    MODEL_SUMMARIZATION: str = Field(default="sshleifer/distilbart-cnn-12-6", json_schema_extra={"env": "MODEL_SUMMARIZATION"})
    ...
    QDRANT_API_KEY: Optional[str] = Field(default=None, json_schema_extra={"env": "QDRANT_API_KEY"})
