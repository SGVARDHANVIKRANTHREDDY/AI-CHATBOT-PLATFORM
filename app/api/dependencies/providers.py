...
def get_llm_provider() -> FallbackProvider:
    primary = HuggingFaceProvider(model_id=settings.HF_MODEL, token=settings.HF_TOKEN, api_url=settings.HF_API_URL)
    fallback = OpenAIProvider(api_key=settings.OPENAI_API_KEY, model=settings.OPENAI_MODEL)
