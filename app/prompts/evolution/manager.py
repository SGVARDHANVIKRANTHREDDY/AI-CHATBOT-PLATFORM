...
from app.prompts.evolution.models import PromptEvolution, PromptVersion, PromptPerformance
...
class PromptEvolutionManager:
    ...
    async def record_feedback(self, prompt_key: str, version_id: str, score: float):
        ...
        if hasattr(evo, 'candidate_version_id') and evo.candidate_version_id == version_id:
            if perf.hit_count >= 5: # Threshold for promotion check
                active_perf = evo.get_active().performance
                if perf.avg_score > active_perf.avg_score:
                    _LOG.info(f"Promoting candidate {version_id} for {prompt_key} (Score: {perf.avg_score} > {active_perf.avg_score})")
                    ...
                    _LOG.warning(f"Rejecting candidate {version_id} for {prompt_key} (Degradation: {perf.avg_score} < {active_perf.avg_score})")
                    evo.candidate_version_id = None
                self._save()

        # Mutation Logic: If active is underperforming, generate a new candidate
        active_ver = evo.get_active()
        if active_ver.performance.hit_count >= 10 and active_ver.performance.avg_score < 0.7:
            if not getattr(evo, 'candidate_version_id', None):
    ...
    def get_prompt_with_id(self, prompt_key: str) -> tuple[str, str]:
        ...
        if candidate_id and random.random() < 0.2:
