...
from app.shared.monitoring import AGENT_EXECUTION_TIME, TOOL_CALL_COUNT
...
from app.swarm.execution import ExecutionStrategy, SequentialExecution, SwarmExecution
...
class ReasoningGraphEngine:
    ...
    async def execute(
        ...
        state: AgentState,
        exec_ctx: Optional[AgentExecutionContext] = None,
    ) -> List[Any]:
        """
    ...
    async def _execute_node(self, node: ReasoningNode, graph: ReasoningGraph, state: AgentState) -> tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            if node.metadata.get("agent"):
                ...
                if match:
                    ...
                    if self._exec_ctx:
                        ...
                        self._exec_ctx.check()
                    with start_span("tool.execute", {"tool.name": name}) as span:
                        ...
                        emit_observability_event(
                            _LOG, event="tool.execute", category="tool",
                            duration_ms=tool_elapsed, tool_name=name,
                        )
            ...
            emit_observability_event(
                _LOG, event="agent.node.complete", category="agent",
                duration_ms=elapsed, node_id=node.id,
                node_type=str(node.type), agent_key=agent_key,
            )
            ...
            from app.orchestrator.watchdog import AgentBudgetExceeded
            if isinstance(e, AgentBudgetExceeded):
                raise
            elapsed = (time.perf_counter() - t0) * 1000
            emit_observability_event(
                _LOG, event="agent.node.error", category="agent",
                duration_ms=elapsed, node_id=node.id, error=str(e),
            )
