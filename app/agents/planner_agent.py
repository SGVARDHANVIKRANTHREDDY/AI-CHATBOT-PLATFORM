...
JSON Output:"""
    ...
    async def plan(self, query: str) -> Tuple[ReasoningGraph, str]:
        ...
        emit_observability_event(
            _LOG, event="agent.plan.start", category="agent",
            prompt_version=version_id, query_length=len(query),
        )
        ...
        try:
            ...
            emit_observability_event(
                _LOG, event="agent.plan.complete", category="agent",
                duration_ms=elapsed, node_count=len(graph.nodes),
            ...
            elapsed = (time.perf_counter() - t0) * 1000
            emit_observability_event(
                _LOG, event="agent.plan.error", category="agent",
